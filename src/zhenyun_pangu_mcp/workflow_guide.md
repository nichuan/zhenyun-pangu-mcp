...
修复包包含原因与目标不变量、核查 SELECT、预期影响行数、修改 SQL、执行后校验 SELECT。各业务域的写入条件以对应 SQL Skill 为准：采购寻源逐条 UPDATE WHERE 只用已核实的 `tenant_id + 目标表主键`，旧状态、版本和单据关联字段在预查中核对，不进入 UPDATE WHERE；盘古履约沿用其旧状态/版本条件。版本递增本身不防止覆盖并发修改。预期行数不能由 LIMIT 截断结果推断；无法证明完整范围时标未确认，停止生成可直接执行的批量修复。
