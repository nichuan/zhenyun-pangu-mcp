...
-- CREATE TABLE IF NOT EXISTS 不会给旧表补列；以下语句兼容历史环境。
ALTER TABLE sql_templates ADD COLUMN IF NOT EXISTS execution_flow TEXT;
ALTER TABLE sql_templates ADD COLUMN IF NOT EXISTS example_case TEXT;
ALTER TABLE sql_templates ADD COLUMN IF NOT EXISTS problem_description TEXT;
ALTER TABLE sql_templates ADD COLUMN IF NOT EXISTS symptom TEXT;
ALTER TABLE sql_templates ADD COLUMN IF NOT EXISTS root_cause TEXT;
ALTER TABLE sql_templates ADD COLUMN IF NOT EXISTS preconditions TEXT;
ALTER TABLE sql_templates ADD COLUMN IF NOT EXISTS diagnosis_steps TEXT;
ALTER TABLE sql_templates ADD COLUMN IF NOT EXISTS verify_sql TEXT;
ALTER TABLE sql_templates ADD COLUMN IF NOT EXISTS rollback_sql TEXT;
ALTER TABLE sql_templates ADD COLUMN IF NOT EXISTS parameters JSONB NOT NULL DEFAULT '{}'::jsonb;
ALTER TABLE sql_templates ADD COLUMN IF NOT EXISTS execution_policy TEXT;
ALTER TABLE sql_templates ADD COLUMN IF NOT EXISTS last_used_at TIMESTAMPTZ;
...
RETURNS TABLE (
    ...
    last_used_at timestamptz, similarity float
)
LANGUAGE sql STABLE
...
RETURNS TABLE (
    ...
    execution_policy text, risk_level text, status text, source_type text,
    verified boolean, verified_at timestamptz, usage_count bigint,
    last_used_at timestamptz, rank float
)
LANGUAGE sql STABLE
SET search_path = public, pg_temp
AS $$
    SELECT t.id, t.title, t.template_no, t.category, t.system,
           ...
           (CASE WHEN t.title ILIKE '%' || keyword || '%' THEN 6.0 ELSE 0 END
            ...
            + CASE WHEN COALESCE(t.verify_sql, '') ILIKE '%' || keyword || '%' THEN 1.0 ELSE 0 END)::float AS rank
    FROM sql_templates t
